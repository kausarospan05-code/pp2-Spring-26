...
# Handle Feb 29 birthdays-kibise jyl
...
candidate_year = cy#current year
...
tz_str = birth_line.split(" UTC")[1]#uakyt beldeudi kesip alady
sign = 1 if tz_str[0] == "+" else -1#uakyt beldeui + or -
hours, minutes = map(int, tz_str[1:].split(":"))#matindi sanga,06:00->6cagat 0 minut
offset = datetime.timedelta(hours=sign*hours, minutes=sign*minutes)#airmashylyq
bday_utc = bday_local - offset#tugan t=kun utc boiunsha

if bday_utc < current_utc:#birthday otip ketti me
    candidate_year += 1
    bday_date = birthday_for_year(candidate_year, bm, bd)
    bday_local = datetime.datetime(candidate_year, bday_date.month, bday_date.day, 0, 0, 0)
...
if diff_seconds == 0:
    ...
    days = int((diff_seconds + 86399) // 86400)  # ceil=1kun kaldy dep
    print(days)

    #Алматыда  басталғанда (түнгі 00:00), Лондонда кешкі сағат 19:00 (алдыңғы күн).